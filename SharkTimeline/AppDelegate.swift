...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    func setupMenuBar() {
        ...
        let quitItem = NSMenuItem(title: "Quit SharkTimeline", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
    ...
    func updateWindowPosition() {
        ...
        if targetScreen == nil {
            ...
            // Clear the preferred screen setting if it's no longer available
            UserDefaults.standard.removeObject(forKey: "preferredScreenIdentifier")
        }
        ...
        let newX = (position == "right") ? (screen.frame.width - window.frame.width) : screen.frame.minX
